#include <math.h>
#include "svf_legacy.h"

#define MIN(x, y) (((x) < (y)) ? (x) : (y))

using namespace daisysp;

void Svf_legacy::Init(float sample_rate)
{
    sr_        = sample_rate;
    fc_        = 200.0f;
    res_       = 0.5f;
...
void Svf_legacy::Process(float in)
{
    input_ = in;
    // first pass
...
void Svf_legacy::SetFreq(float f)
{
    if(f < 0.000001f)
    {
        fc_ = 0.000001f;
...
void Svf_legacy::SetRes(float r)
{
    if(r < 0.0f)
    {
        r = 0.0f;
